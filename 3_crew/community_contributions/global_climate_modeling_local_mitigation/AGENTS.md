...
## Project-Specific: Global Climate Modeling Local Mitigation

### 5-Agent System Architecture

This project implements a multi-agent climate mitigation planning system:

| Agent | Role | Focus |
|-------|------|-------|
| `team_lead` | Chief Architect & Strategist | Orchestrates workflow, validates scientific accuracy |
| `data_engineer` | Climate Data Engineer | Ingests/fuses satellite, IoT, weather, socio-economic data |
| `climate_modeler` | Climate Modeling Agent | Runs CMIP6 models + ML surrogates at 10km resolution |
| `impact_analyst` | Impact & Risk Agent | Translates forecasts to local risks (flood, crop, health) |
| `solution_architect` | Solution Architect | Designs green infrastructure, microgrids, retreat plans |

### Workflow (Sequential Process)

```
data_ingestion_task → climate_modeling_task → impact_assessment_task → solution_design_task → final_review_task
```
...
|-----------|-------------|---------|
| `region` | Geographic region for analysis | `"Mekong Delta"`, `"Miami Beach"`, `"Sahel Region"` |
| `current_year` | Auto-injected for temporal context | `"2026"` |
