"""Agentic research manager: uses planner, search, writer, evaluator, optimizer as tools and hands off to email."""
...
5. **Handoff**: Hand off the final report to the Email agent. If the user provided a "Recipient email: ..." in the query, include that at the start of what you pass to the Email agent so the report is sent to that address. Otherwise the Email agent will use its default recipient.

**Rules:**
- Use only the tools and handoff provided; do not make up content.
...
- Hand off exactly once to the Email agent with the final report text (and recipient email if provided).
