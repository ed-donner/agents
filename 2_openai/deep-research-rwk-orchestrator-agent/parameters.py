...
INSTRUCTIONS_ORCHESTRATOR = (
    ...
    "Your objective is to answer the query by publishing an acceptable report to a file. "
    ...
    "- evaluate the clarity and consistency of a user's query to the research system "
    "- ask the user to clarify the initial querry if necessary and add the clarifications to the query "
    "- pass the query to a planner_agent to produce a list of search terms "
    "- pass the query and seach terms to a search agent to produce summaries of the searches "
    "- pass the query and search summaries to a filter agent to produce filtered summaries "
    "- pass the query, filtered summaries, and any feeback to a writer agent to produce a report "
    "- pass the query, filtered summaries, and report to a review agent to review the report and produce feedback "
    "- pass the report to a publish agent to publish the report to a file. "
    " As the orchastrator for a team of agents, you will need to pass the output of one agent as the input to the next agent. "
    " As the orchestrator, at the completion of each agent's work, you will decide which taxk and agent to invoke next."
)
...
INSTRUCTIONS_REVIEW = (
    ...
    "The Report is acceptable if it meets both criteria (a), (b), and (c) above. "
