...
- `world.py` now decides after each successful Agent creation whether to create a new Creator version, using `CHANCES_THAT_I_CHANGE_CREATOR = 0.5`.
- When a new Creator is created, later Agents are generated by that Creator until the chance roll creates another Creator.
...
Creator changes are intentionally probabilistic. After each successful Agent creation, `world.py` rolls `CHANCES_THAT_I_CHANGE_CREATOR`. If the roll succeeds, the current Creator writes and registers the next `creatorN.py`, and that new Creator becomes responsible for the following Agents.
...
9. `world.py` rolls the Creator-change chance.
10. If the roll succeeds, the active Creator creates and registers the next `creatorN.py`.
11. Future Agents are created by the newest active Creator until another chance roll creates another Creator.
