...
from database import read_log, read_risk
...
class Trader:
    ...
    def get_risk_status(self) -> str:
        ...
        limits = (
            f"VaR:{risk['var_limit']:.0%} | "
            f"MaxPos:{risk['max_position_pct']:.0%} | "
            f"DailyLoss:{risk['daily_loss_limit']:.0%}"
        )
        if cb:
            last = events[-1]["event"] if events else "No details"
            status_html = f"<span style='color:red;font-weight:bold'>🔴 HALTED</span> — {last}"
        else:
            status_html = "<span style='color:lime;font-weight:bold'>🟢 TRADING</span>"

        recent = ""
        for e in events[-3:]:
            sev = e.get("severity", "")
            sev_color = {"critical": "red", "high": "orange", "medium": "yellow", "low": "lime"}.get(sev, "white")
            recent += f"<span style='color:{sev_color}'>[{sev}] {e['timestamp'][:16]} {e['event']}</span><br/>"

        return (
            f"<div style='padding:6px;font-size:12px;'>"
            f"<b>Risk Manager</b> | {status_html}<br/>"
            f"<span style='color:#aaa'>{limits}</span><br/>"
            f"{recent}</div>"
        )
