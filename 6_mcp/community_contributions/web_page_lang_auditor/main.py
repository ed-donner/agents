...
async def main():
    ...
    # Automatically detect the first CSV file in the input directory
    csv_files = [f for f in os.listdir(input_dir) if f.lower().endswith(".csv")]
    if not csv_files:
        raise FileNotFoundError(f"No CSV files found in {input_dir}")
    csv_filename = csv_files[0]  # pick the first CSV automatically
    print(f"Detected CSV file: {csv_filename}")

    # MCP filesystem server (handles reading input / writing output)
    files_params = {
        "command": "npx",
        "args": ["-y", "@modelcontextprotocol/server-filesystem", input_dir, output_dir]
    }

    # MCP web auditor server
